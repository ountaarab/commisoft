...
class Locations extends CI_Controller
{
   ...
   public function read($id) 
        {
            $data ['siap'] = $this->Locations_model->select_by_id($id);
    ...
    public function create() 
        {
            $data = array(
                            ...
                            'id_projects'        => set_value('list_project'),
                            'location_id'        => set_value('location_id'),
                            'location_name'      => set_value('location_name'),
                        );
            
            $data ['list_project'] = $this->Locations_model->select_m_project ();
    ...
    public function update($id) 
        {
            ...
            if ($row) {
                $data = array(
                                    ...
                                    'id_projects'         => set_value('list_project', $row->id_projects),
    ...
    public function update_action() 
        {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->Locations_model->update($this->input->post('id_location', true), $data);

                date_default_timezone_set('Asia/bangkok');
                $datetime = date('Y-m-d H:i:s');
                $datalog = array(
                                    'id_projects'         => $this->input->post('list_project',true),
                                    'id_locations'        => $this->input->post('id_location', true),
                                    'location_id'         => $this->input->post('location_id',true),
                                    'location_name'       => $this->input->post('location_name',true),
                                    'id_users'            => $this->session->userdata('id_users',true),
                                    'note'                => 'update',

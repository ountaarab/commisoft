...
<!-- 		<th>Sub No</th> -->
		...
		<th width="200">Action</th>
            ...
            foreach ($subs_data as $subs)
            {
                ?>
                <tr>
			 <td width="10px"><input type="checkbox" class="checkitem" name="pilih[]" value="<?=$subs->id?>"></td>
...
<!-- 			<td><?php echo $subs->sub_no ?></td> -->
			...
			<td style="text-align:center">
				<?php 
				echo anchor(site_url('subs/read/'.$subs->id),'<i class="icon-desktop" aria-hidden="true"></i>'); 
				echo '  '; 
				echo anchor(site_url('subs/update/'.$subs->id),'<i class="icon-edit" aria-hidden="true"></i>'); 
				echo '  '; 
				echo anchor(site_url('subs/delete/'.$subs->id),'<i class="icon-trash" aria-hidden="true"></i>','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'); 
				?>
			</td>
		</tr>
                <?php

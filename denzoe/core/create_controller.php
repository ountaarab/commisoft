...
$string .= "\n\n    public function index()
    {
       \$q = urldecode(\$this->input->get('q', TRUE));
        ...
        if (\$q <> '') {
            \$config['base_url'] = base_url() . 'index.php/$c_url/index?q=' . urlencode(\$q);
            \$config['first_url'] = base_url() . 'index.php/$c_url/index?q=' . urlencode(\$q);
        } else {
...
$string .= "\n\t    );
     ...
     public function delete_select() 
    {
        ...
        \$jumlah = count(\$pilih);
        for(\$i=0; \$i < \$jumlah;\$i++){
            \$this->db->query(\"DELETE FROM $table_name WHERE $pk = \".\$pilih[\$i].\"\");
    ...
    \$file_name = '" . $c_url . "-'.md5(rand()) . '.pdf';
    \$html_code = \$this->load->view('" . $c_url . "/". $v_doc . "', \$data, true);
    \$pdf = new Pdf('A4');
...
/* Generated by Denzoe Codeigniter CRUD Generator ".date('Y-m-d H:i:s')." */
